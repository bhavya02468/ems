    ...
    .then((emailresult) => {
      if (!emailresult) {
        throw customExc.completeCustomException("admin_not_found", false);
          ...
          let tokenObj = buildforgotTokenGenObj(email);
...
function emailsend(email,token){
  ...
  let transporter = nodemailer.createTransport({
    ...
    requireTLS: true,
    auth: {
      user: 'bhavyamehta6c@gmail.com',
      pass: 'oxysplunnjrxpepc'
    }
  });

  let mailOptions = {
    from: 'bhavyamehta6c@gmail.com',
    to: email,
