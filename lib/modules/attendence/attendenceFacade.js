...
function getEmployeeAttendance(req){
  let pge_no = parseInt(req.query.page_no) ? parseInt(req.query.page_no) : 0;
  let limit = parseInt(req.query.limit) ? parseInt(req.query.limit) : 30;
  let query = [{}];
  let column ="date"
  ...
  let startdate=new Date(req.query.first)

  let enddate=new Date(req.query.last)
  ...
  if(true){
    query.push({
      ...
      query.push({empId: mongoose.Types.ObjectId(req.query._id)});
